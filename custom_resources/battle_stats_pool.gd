...
var total_weights_by_tier := [0.0, 0.0]
...
func get_random_battle_for_tier(tier: int) -> BattleStats:
	print("Getting random battle for tier " + str(tier))
	var roll := RNG.instance.randf_range(0.0, total_weights_by_tier[tier])
	var battles := _get_all_battles_for_tier(tier)
	
	for battle: BattleStats in battles:
		if battle.accumulated_weight > roll:
			assert(battle, "No battle found for tier " + str(tier))
			print("Found battle " + battle.resource_name)
			return battle
	assert(false, "No battle found for tier " + str(tier))
	return null
