...
const EVENT_ROOM_WEIGHT := 0
const SHOP_ROOM_WEIGHT := 1.5
const ELITE_ROOM_WEIGHT := 3
...
func _setup_boss_room() -> void:
	...
	boss_room.battle_stats = battle_stats_pool.get_random_battle_for_tier(1)
	boss_room.value = 100
...
func _setup_room_types() -> void:
	...
	for room: Room in map_data[floori( FLOORS / 2.0)]:
		if room.next_rooms.size() > 0:
...
func _set_room_randomly(room_to_set: Room) -> void:
	var campfire_below_2 := true
	var consecutive_campfire := true
	var consecutive_shop := true
	var campfire_on_third_last := true
	
	var type_candidate: Room.Type
	
	while campfire_below_2 or consecutive_campfire or consecutive_shop or campfire_on_third_last:
		...
		campfire_below_2 = is_campfire and room_to_set.row < 2
		consecutive_campfire = is_campfire and has_campfire_parent
		consecutive_shop = is_shop and has_shop_parent
		campfire_on_third_last = is_campfire and room_to_set.row == FLOORS - 3
