...
@export var shop_relics: Array[Relic] = [
	...
	preload('res://relics/healing_potion.tres'),
	preload('res://relics/mana_potion.tres'),
...
func _generate_shop_relics() -> void:
	...
	shop_relics_array = available_relics.slice(0, randi() % 3 + 1)	
...
func _get_updated_shop_cost(original_cost: int) -> int:
	return modifier_handler.get_modified_value_by_type(original_cost, Modifier.Type.SHOP_COST)
...
func _on_shop_relic_bought(relic: Relic, gold_cost: int) -> void:
	...
	if relic is CouponsRelic:
		...
		coupons_relic.add_shop_modifier(self, null)
		_update_item_costs()
	else:
		_update_items()
