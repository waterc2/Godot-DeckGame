...
func _show_card_rewards() -> void:
	if not run_stats or not character_stats:
		print("Run stats or character stats not set")
		return
	...
	# Start to generate card rewards by rarity
...
func _get_random_available_card(available_cards: Array[Card], with_rarity: Card.Rarity) -> Card:
	...
	if all_possible_cards.size() == 0:
		# If the filtered list is empty, return a random card from the original list
		return RNG.array_pick_random(available_cards)
	else:
		# Otherwise, return a random card from the filtered list
