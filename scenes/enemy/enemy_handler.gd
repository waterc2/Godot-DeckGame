...
# KEY: start to apply statuses to the enemy
func _on_enemy_statuses_applied(type: Status.Type, enemy: Enemy) -> void:
	match type:
		Status.Type.START_OF_TURN:
			enemy.do_turn()
		Status.Type.END_OF_TURN:
			#  KEY: remove from acting_enemies because the enemy has finished its turn
